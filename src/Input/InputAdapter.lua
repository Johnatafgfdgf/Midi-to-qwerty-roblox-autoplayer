...
local function spec(token)
    ...
    local vk = string.byte(upper)
    if not (base:match("[%a%d]")) then return nil end
    local enumName = base:match("%d") and digitEnum[base] or upper
    return {token = token, base = base, shift = shift, vk = vk, enumName = enumName, physical = upper}
end
...
function InputAdapter:_send(down, s)
    ...
    elseif self.backend == "VirtualInputManager" then
        ...
        local ok, err = pcall(self.vim.SendKeyEvent, self.vim, down, keyCode, false, game)
        return ok, err
    end
...
function InputAdapter:press(token)
    ...
    local ok, err = self:_send(true, s)
