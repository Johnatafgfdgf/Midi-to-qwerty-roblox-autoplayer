...
function InputAdapter.new()
    local self=setmetatable({shiftRefs=0,strikeGen={},heldStrike={},destroyed=false},InputAdapter)
...
function InputAdapter:physicalId(token)
    local s=spec(token);return s and s.physical or tostring(token)
end
function InputAdapter:_sendKey(down,s)
    if self.destroyed then return false,"adapter destroyed" end
...
function InputAdapter:strike(token,opts)
    ...
    -- Previous builds clamped this to 260 ms, silently truncating long MIDI notes.
    local holdMs=math.clamp(tonumber(opts.holdMs) or 34,6,2500)
...
function InputAdapter:destroy()
    self:releaseExpressive();self:releaseModifiers();self.destroyed=true
end
function InputAdapter:diagnostics()
    return {backend=self.backend,available=self.backend~="Unavailable",nativeVelocity=self.velocityHook~=nil,expressiveStrike=true,maxStrikeHoldMs=2500}
end
return InputAdapter
