...
local function clone(n)
    ...
    if n.parts then c.parts={};for k,v in pairs(n.parts)do c.parts[k]=v end end
    return c
end
...
local function smartShift(notes,profile,base)
    local baseCoverage=coverage(notes,profile,base,0);local best,bestCoverage=0,baseCoverage
    for _,s in ipairs({-24,-12,12,24})do local c=coverage(notes,profile,base,s);if c>bestCoverage+1e-9 then best,bestCoverage=s,c end end
...
local function normalizedVelocity(n)
    local v=n.expressiveVelocity or n.velocity or n.originalVelocity or .7;if v>1 then v=v/127 end;return math.clamp(v,0,1)
end
...
function Mapper.mapNotes(notes,profile,settings)
    ...
    if range=="SmartOctave"then smart,stats.smartCoverage=smartShift(notes,profile,transpose);range="OctaveFold"end;stats.smartTranspose=smart
    ...
    for _,n in ipairs(deduped)do n.__idx=nil end;stats.mapped=#deduped;stats.coverage=stats.total>0 and(#deduped/stats.total)or 1
    return deduped,stats
end

local function articulationFactor(n,influence)
    influence=math.clamp(influence or .82,0,1);local target=1
    if n.articulation=="Staccato"then target=.50 elseif n.articulation=="Legato"then target=.98 elseif n.articulation=="Accent"then target=.84 elseif n.articulation=="Sustain"then target=.96 end
...
local function humanizedRelease(n)
    ...
    if shiftedKeyRelease and endTime then
        ...
        -- Prefer finger release, but never let it predate the humanized attack.
...
local function holdFromMidi(n,expr,nextSameStart)
    expr=expr or {};local start=n.startTime or 0;local releaseTime=humanizedRelease(n);local midiMs=math.max(1,(releaseTime-start)*1000);local mode=expr.durationMode or"MIDI";local hold
    if mode=="Fixed"then hold=expr.fixedHoldMs or 42
    else
        hold=midiMs*math.clamp(expr.holdScale or .96,.1,1.35);hold*=articulationFactor(n,expr.articulationInfluence)
        local v=normalizedVelocity(n);local velInf=math.clamp(expr.velocityInfluence or .14,0,1);hold*=1+(v-.5)*.10*velInf
    end
    local minHold=math.clamp(expr.minHoldMs or 14,6,250);local maxHold=math.clamp(expr.maxHoldMs or 1400,minHold,2400);hold=math.clamp(hold,minHold,maxHold)
    if nextSameStart then local gap=math.max(3,expr.releaseGapMs or 7);local available=(nextSameStart-start)*1000-gap;if available>0 then hold=math.min(hold,math.max(6,available))end end
    return hold
end
local function eventRank(e)if e.action=="up"then return 1 end;if e.action=="strike"or e.action=="tap"then return 2 end;return 3 end
function Mapper.toEvents(mappedNotes,settings)
    settings=settings or {};local expr=settings.expression or{};local events={};local nextByToken={};local nextSame={}
    for i=#mappedNotes,1,-1 do local n=mappedNotes[i];nextSame[i]=nextByToken[n.token];nextByToken[n.token]=n.startTime end
    for i,n in ipairs(mappedNotes)do
        if settings.triggerMode=="Hold"then
            events[#events+1]={time=n.startTime,action="down",token=n.token,note=n,velocity=normalizedVelocity(n)};events[#events+1]={time=humanizedRelease(n),action="up",token=n.token,note=n,velocity=normalizedVelocity(n)}
        else
            local action=expr.enabled==false and"tap"or"strike";events[#events+1]={time=n.startTime,action=action,token=n.token,note=n,velocity=normalizedVelocity(n),holdMs=holdFromMidi(n,expr,nextSame[i]),nativeVelocity=expr.nativeVelocity~=false}
        end
    end
