...
function Scheduler.new(noteManager)
    ...
    s.noteManager=noteManager
    s.events={};s.index=1;s.duration=0;s.position=0;s.speed=1;s.playing=false;s.paused=false
    ...
    s.lastUi=0;s.uiInterval=.04
    return s
end

function Scheduler:setEvents(events,duration,rebuildAt)
    self:stop(false)
    self.events=events or {};self.duration=duration or 0;self.rebuildAt=rebuildAt
    self.index=1;self.position=0
    self.stats={processed=0,skipped=0,late=0,driftSumMs=0,driftPeakMs=0,catchups=0}
end
function Scheduler:setOptions(o)
    o=o or {}
    self.maxLateMs=o.maxLateMs or self.maxLateMs
    self.lateMode=o.lateMode or self.lateMode
    self.loopSong=o.loopSong==true
end
function Scheduler:setAB(a,b)
    if a and b and b>a then
        self.loopA,self.loopB=math.clamp(a,0,self.duration),math.clamp(b,0,self.duration)
    else self.loopA,self.loopB=nil,nil end
end
...
function Scheduler:_process(p)
    while self.index<=#self.events do
        ...
        elseif e.action=="strike" then
            -- Event times are in musical time. A 400 ms note at 0.5x must remain held
            -- for about 800 real ms, while at 2x it should be about 200 real ms.
            local scaledHold=(e.holdMs or 34)/math.max(.05,self.speed)
            self.noteManager:strike(e.token,{velocity=e.velocity,holdMs=scaledHold,nativeVelocity=e.nativeVelocity,note=e.note})
...
function Scheduler:_connect()
    ...
    self.connection=RunService.Heartbeat:Connect(function()
        ...
        if self.onPosition and now-self.lastUi>=self.uiInterval then
            self.lastUi=now;self.onPosition(math.min(p,self.duration),self.duration,self.stats)
        end
...
function Scheduler:play()
    ...
    self.playing,self.paused=true,false;self.lastUi=0;self:_connect()
end
...
function Scheduler:destroy()
    self:stop();self.onPosition=nil;self.onEvent=nil;self.onFinished=nil;self.onSpeed=nil
end
return Scheduler
