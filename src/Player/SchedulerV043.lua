...
function Scheduler.new(noteManager)
    ...
    s.loopSong=false;s.loopA=nil;s.loopB=nil
    s.maxLateMs=120
    s.lateMode="CatchUp"
...
function Scheduler:_shouldSkip(e,lateMs)
    -- Fidelity build: CatchUp is the default and never drops note attacks.
...
function Scheduler:_process(p)
    ...
    if processedThisBeat>8 then self.stats.catchups+=1 end
end
...
function Scheduler:_connect()
    ...
    self.connection=RunService.Heartbeat:Connect(function()
        ...
        if self.onPosition and os.clock()-self.lastUi>=.1 then
