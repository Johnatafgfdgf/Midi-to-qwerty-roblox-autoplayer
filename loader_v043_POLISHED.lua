...
local PINNED_COMMIT="1bde113aaf682cbe7fc3d9c69785de1448a6709f"
local VERSION="0.4.3-POLISHED"
...
local REMAP={
    ["Main"]="MainV043",
    ["MainBase"]="Main",
    ["ConfigDefaults"]="ConfigDefaultsV043",
    ["UI/App"]="UI/AppMobileV043",
    ["Piano/Mapper"]="Piano/MapperV043",
    ["Player/Scheduler"]="Player/SchedulerV043",
}
...
local function Require(path)
    ...
    loading[path]=true
    local remote=REMAP[path] or path
    local url=base..remote..".lua"
    local ok,source=pcall(function()return game:HttpGet(url)end)
    if not ok or type(source)~="string" then
        loading[path]=nil
        error("[MIDIQWERTY] Download failed: "..remote.." | "..tostring(source),2)
    end
    local chunk,err=loadstring(source,"=MIDIQWERTY/"..remote)
    if not chunk then
        loading[path]=nil
        error("[MIDIQWERTY] Compile failed: "..remote.." | "..tostring(err),2)
    end
